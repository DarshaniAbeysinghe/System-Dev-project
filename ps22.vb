...
Public Class ps22
    ...
    Private Sub ps22_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadlist()
    End Sub
    ...
    Private Sub picc_click(sender As Object, e As EventArgs)
        ...
        If clickedPictureBox.Enabled Then
            ...
            Dim dt As New DataTable()
            Dim query As String = "SELECT `ID`,`First_Name` FROM `pradeshiya_saba_election_candidate` WHERE `First_Name` = @PartyName"
    ...
    Sub loadlist()
        ...
        Try
            ...
            dbconn.Open()
            Dim selectedParty As String = ps1.Dgv1.CurrentRow.Cells("Name").Value.ToString()
            Dim selectedpradeshiya_saba As String = ps1.Dgv2.CurrentRow.Cells("pradeshiya_saba").Value.ToString()
            cmd = New MySqlCommand("SELECT `Img`,`ID`,`First_Name`FROM pradeshiya_saba_election_candidate WHERE Party=@Party AND pradeshiya_saba=@pradeshiya_saba", dbconn)
            cmd.Parameters.AddWithValue("@Party", selectedParty)
            cmd.Parameters.AddWithValue("@pradeshiya_saba", selectedpradeshiya_saba)
    ...
    Private Sub Guna2GradientButton4_Click(sender As Object, e As EventArgs) Handles Guna2GradientButton4.Click
        Try
            ...
            For j As Integer = 0 To Dgv3.Rows.Count - 2
                Dim query As String
                query = "INSERT INTO psvoting_details_2(Nic, Time,Selected_Candidate) VALUES (@Nic, @Time,@Selected_Candidate)"
