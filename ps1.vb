...
Public Class ps1
    ...
    Private Sub ps1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadlist()
    End Sub
    ...
    Private Sub LoadDataForSelectedPictureBox()
        If selectedPictureBox IsNot Nothing Then
            ...
            ' Load voter data into dtVoter
            Dim voterQuery As String = "SELECT `First_Name`, `pradeshiya_saba` FROM `voter_registration` WHERE Nic = @Nic"
            Using cmdVoter As New MySqlCommand(voterQuery, dbconn)
                cmdVoter.Parameters.AddWithValue("@Nic", Psloging.Txtnic.Text)
    ...
    Sub update_votervotestatus()
        Try
            ...
            Dim query As String
            query = "UPDATE voter_registration SET psstatus=@psstatus WHERE Nic=@Nic"
            cmd = New MySqlCommand(query, dbconn)
            cmd.Parameters.Clear()
            cmd.Parameters.AddWithValue("@psstatus", "Voted")
            cmd.Parameters.AddWithValue("@Nic", value:=Psloging.Txtnic.Text)
    ...
    Private Sub Guna2GradientButton4_Click(sender As Object, e As EventArgs) Handles Guna2GradientButton4.Click
        Try
            ...
            For j As Integer = 0 To Dgv1.Rows.Count - 2
                Dim query As String
                query = "INSERT INTO psvoting_details(Nic, Time,pradeshiya_saba,Selected_Party) VALUES (@Nic, @Time,@pradeshiya_saba, @Selected_Party)"
                cmd = New MySqlCommand(query, dbconn)
                cmd.Parameters.Clear()
                cmd.Parameters.AddWithValue("@Nic", Psloging.Txtnic.Text)
                cmd.Parameters.AddWithValue("@pradeshiya_saba", Dgv2.Rows(j).Cells(1).Value)
                cmd.Parameters.AddWithValue("@Time", Date.Now.ToString("yyyy-MM-dd HH:mm:ss"))
                cmd.Parameters.AddWithValue("@Selected_Party", Dgv1.Rows(j).Cells(0).Value) ' Use Dgv1.Rows(j) to access the current row
