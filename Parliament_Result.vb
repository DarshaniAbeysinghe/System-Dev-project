...
Public Class Provincial_Result
    ...
    Dim i As Integer
    Private Sub Provincial_Result_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        load_result()
        load_results()
        Dim sum As Integer
        Dim max As Integer
    ...
    Sub load_result()
        dbconn.Open()
        cmd = New MySqlCommand("SELECT `Time` as Time,`Selected_Party` as Selected_Party,COUNT(*) as VOTE FROM `parvoting_details` group BY `Selected_Party` HAVING(COUNT(*)>=1) ORDER BY `VOTE`DESC", dbconn)
    ...
    Sub load_results()
        ...
        cmd = New MySqlCommand("SELECT `Time` as Time,`Selected_Candidate` as Selected_Candidate,COUNT(*) as VOTE FROM `parvoting_details_2` group BY `Selected_Candidate` HAVING(COUNT(*)>=1) ORDER BY `VOTE`DESC", dbconn)
