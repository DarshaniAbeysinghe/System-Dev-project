...
Public Class ParVote
    ...
    Private Sub ParVote_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadlist()
    ...
    Private Sub picc_click(sender As Object, e As EventArgs)
        ...
        If clickedPictureBox.Enabled Then
            ...
            Dim dt As New DataTable()
            Dim query As String = "SELECT `ID`,`First_Name` FROM `Parliament_election_candidate_registration` WHERE `First_Name` = @PartyName"
    ...
    Sub loadlist()
        ...
        Try
            ...
            dbconn.Open()
            Dim selectedParty As String = Par1Vote.Dgv1.CurrentRow.Cells("Name").Value.ToString()
            Dim selectedDistrict As String = Par1Vote.Dgv2.CurrentRow.Cells("District").Value.ToString()
            cmd = New MySqlCommand("SELECT `Img`,`ID`,`First_Name`FROM Parliament_election_candidate_registration WHERE Party=@Party AND District=@District", dbconn)
    ...
    Private Sub Guna2GradientButton4_Click(sender As Object, e As EventArgs) Handles Guna2GradientButton4.Click
        Try
            ...
            For j As Integer = 0 To Dgv3.Rows.Count - 2
                Dim query As String
                query = "INSERT INTO parvoting_details_2(Nic, Time,Selected_Candidate) VALUES (@Nic, @Time,@Selected_Candidate)"
                cmd = New MySqlCommand(query, dbconn)
                cmd.Parameters.Clear()
                cmd.Parameters.AddWithValue("@Nic", Loging_Form_Par.Txtnic.Text)
